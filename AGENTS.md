...
| M1 session | **Flight is playable.** `sim/math.ts`, `sim/flight.ts` (helicopter integrator), `sim/step.ts`, `game/Loop.ts` (fixed timestep), `input/InputManager.ts` + `bindings.ts` (mouse-stick and full-keyboard), `render/Camera.ts` (spring follow), `views/BeeView.ts` (procedural wing flap), `scenery/Hive.ts`, `ui/hud.ts`. 42 tests. First hands-on playtest produced four fixes: top speed almost halved, stabilization ramp roughly halved, glide cut from ~20 m to ~1.7 m, and a real sign bug in roll (positive `attitude.roll` banks *left*, because Rz(+θ) raises the right wing). |
